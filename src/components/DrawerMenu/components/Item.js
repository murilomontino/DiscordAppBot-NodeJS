...
const Item = ({ Icon, firstPage, name, arrow, className, title, ...rest }) => {
    ...
    return (
        ...
        <div ref={selectPageFirst} title={title} className={className} onClick={arrow? ()=>{}: (e)=>{ItemMenuSelection(e)}} >
        
        <ListItem className={arrow? 'arrow toolbar': 'toolbar'} button {...rest}>

            <ListItemIcon> <Icon className='iconButton' /> </ListItemIcon>
